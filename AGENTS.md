...
- Nested cards are allowed (max 2 levels) per `.cursor/rules/cal-theme.mdc`. Still no decorative hero, gradient-orb backgrounds, or marketing landing unless explicitly requested.
...
Rules:
- Follow design.md component contracts unless `.cursor/rules/cal-theme.mdc` overrides (nested cards max 2, light/dark, Inter tabular-nums).
- Include loading, empty, error, permission, locked, and sync states where applicable.
- Nested cards max 2; no decorative hero, no generic SaaS filler, no marketing landing.
...
| Nests cards more than 2 levels | Flatten to outer + inner only (`.cursor/scopes/cal-theme/10-nested-cards.md`). |
...
1. **Design judge** — `design.md` + `knowledge.md` + `.cursor/rules/cal-theme.mdc` vs the touched UI. Fail **>2** nested card levels, always-open editors that should be dialogs, missing lock copy, Plan vs Log confusion. Do not fail 2-level nested cards.
