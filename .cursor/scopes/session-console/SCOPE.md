...
1. **Hover overlay** — do **not** cover session/note cards. Actions sit in leftover space **inside** the day (usually under the chips). Do **not** overflow to the right onto a neighbor. **No blur, no scrim.** Session/note chips rest flat; the chip under the pointer uses `--cal-shadow-lift`.
