...
- **Live updates**: Calendar/inspector subscribe to `GET /api/athletes/{id}/live` (`WORKOUT_SYNCED`). REST set/session writes emit the same domain event as sync.
...
- **Surfaces**: Calendar = month grid with session chips; hover `+` / `n` for quick-create; chip opens the main-surface SetGrid (`#/calendar?session=<id>`, calendar hidden). Day note opens a popover card, not the editor. Sessions = list mode of the same data, grouped by Block/Week. SetGrid is the prescription/log sheet (keyboard cell model). Insights = composable cards (saved configs, server query); presets ship as data, not hardcoded chart components. Coach Calendar/Sessions/Insights follow the sidebar athlete switcher. Insight-card offline flush uses `mutation_type: insight_card` (no fake workout_id).
- **Math pin**: `MATH_VERSION` is the same string in `backend/math_utils.py` and `src/services/mathEngine.ts`. Shared set vectors live in `tests/math_vectors.json`. Sync 409s on mismatch. Stale-math banner if versions differ after catalog/query/sync.
- **Add lift**: Structured picker (category, search, exercise, variation chips) from the inspector. Movement pattern is a stored catalog field (`movement_pattern`) edited with a structured select.
