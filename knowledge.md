...
- **Math pin**: `MATH_VERSION` (`linear-decay-v2`) is the same string in `backend/math_utils.py` and `src/services/mathEngine.ts`. Shared set vectors live in `tests/math_vectors.json`. Sync 409s on mismatch. e1RM uses RPE-compensated linear decay with `E1RM_RPE_FLOOR = 5.0` (formula input only; missing RPE still returns load).
