...
export function HeatmapChart({ result }: { result: QueryResult }) {
  const matrix = result.matrix;
  if (!matrix) return <p className="text-[11px] text-[var(--cal-muted)]">No heatmap</p>;
  const max = Math.max(1, ...matrix.rows.flatMap((row) => matrix.cols.map((col) => matrix.cells[row]?.[col] || 0)));
  return (
    <div className="overflow-x-auto" role="img" aria-label="Heatmap">
      <table className="w-full text-[10px] font-mono">
        <thead>
          <tr>
            <th className="text-left text-[var(--cal-muted)] font-normal pr-2"> </th>
            {matrix.cols.map((col) => (
              <th key={col} className="text-[var(--cal-muted)] font-normal px-1">{col.slice(-5)}</th>
            ))}
          </tr>
        </thead>
        <tbody>
