...
export function HeatmapChart({ result }: { result: QueryResult }) {
  ...
  if (!matrix) return <p className="text-mini text-fg-muted">No heatmap</p>;
  ...
  return (
    <div className="overflow-x-auto" role="img" aria-label="Heatmap">
      <table className="w-full text-micro font-mono">
        <thead>
          <tr>
            <th className="text-left text-fg-muted font-normal pr-2"> </th>
            {matrix.cols.map((col) => (
              <th key={col} className="text-fg-muted font-normal px-1">{col.slice(-5)}</th>
            ))}
          </tr>
        </thead>
        <tbody>
