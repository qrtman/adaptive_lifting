...
export function HeatmapChart({ result }: { result: QueryResult }) {
  ...
  if (!matrix) return <p className="text-[11px] text-[#AEAEB2]">No heatmap</p>;
  ...
  return (
    <div className="overflow-x-auto" role="img" aria-label="Heatmap">
      <table className="w-full text-[10px] font-mono">
        <thead>
          <tr>
            <th className="text-left text-[#AEAEB2] font-normal pr-2"> </th>
            {matrix.cols.map((col) => (
              <th key={col} className="text-[#AEAEB2] font-normal px-1">{col.slice(-5)}</th>
            ))}
          </tr>
        </thead>
        <tbody>
