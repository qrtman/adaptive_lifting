...
test('hover New session opens a dialog; cancel creates nothing', async ({ page, request }) => {
  ...
  await page.getByTestId('new-session-title').fill('Second day');
  await fillCombo(page, 'new-session-block', 'Hypertrophy');
  await fillCombo(page, 'new-session-week', 'Week1');
  await page.getByTestId('new-session-create').click();
  await expect(page.getByTestId('session-labels')).toHaveText('Hypertrophy · Week1');
  ...
  await fillCombo(page, 'session-block', 'Meet');
  await fillCombo(page, 'session-week', 'Week2');
