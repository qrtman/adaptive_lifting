...
test.describe('calendar week boards', () => {
  ...
  test('places undated sessions on the week board', async ({ page }) => {
    await expect(page.getByTestId('assign-micro-z-w3')).toBeVisible();
    await expect(page.getByTestId('calendar-day-z-w3-2026-09-01').locator('[data-testid^="workout-card-"]')).toBeVisible();
  ...
  test('adds a session with a named control and moves it within the same week', async ({ page }) => {
    await page.getByTestId('assign-micro-z-w3').click();
    await page.getByTestId('add-session-day-z-w3-2026-09-05').click();
    const card = page.getByTestId('calendar-day-z-w3-2026-09-05').locator('[data-testid^="workout-card-"]');
    await expect(card).toBeVisible();
    const testId = await card.getAttribute('data-testid');
    expect(testId).toBeTruthy();
    await html5Drag(page, testId!, 'calendar-day-z-w3-2026-09-06');
    await expect(page.getByTestId('calendar-day-z-w3-2026-09-06').locator(`[data-testid="${testId}"]`)).toBeVisible();
  });

  test('rejects a drop onto another week', async ({ page }) => {
    await page.getByTestId('assign-micro-z-w3').click();
    await page.getByTestId('add-session-day-z-w3-2026-09-05').click();
    const first = page.getByTestId('calendar-day-z-w3-2026-09-05').locator('[data-testid^="workout-card-"]');
    await expect(first).toBeVisible();
    const firstId = await first.getAttribute('data-testid');

    await html5Drag(page, firstId!, 'calendar-day-z-w5-2026-09-16');
    await expect(page.getByTestId('calendar-boundary-lock')).toBeVisible();
    await expect(page.getByTestId('calendar-day-z-w3-2026-09-05').locator(`[data-testid="${firstId}"]`)).toBeVisible();
    await expect(page.getByTestId('calendar-day-z-w5-2026-09-16').locator(`[data-testid="${firstId}"]`)).toHaveCount(0);
  });
  ...
  test('lets the coach widen week dates and drag inside the new range', async ({ page }) => {
    ...
    await expect(card).toBeVisible();
    const testId = await card.getAttribute('data-testid');
    await html5Drag(page, testId!, 'calendar-day-z-w3-2026-09-16');
    await expect(page.getByTestId('calendar-boundary-lock')).toHaveCount(0);
    await expect(page.getByTestId('calendar-day-z-w3-2026-09-16').locator(`[data-testid="${testId}"]`)).toBeVisible();
  });
  ...
  test('deletes a session from the week board', async ({ page }) => {
    ...
    await expect(card).toBeVisible();
    const testId = await card.getAttribute('data-testid');
    const workoutId = testId!.replace('workout-card-', '');
    await page.getByTestId(`delete-session-${workoutId}`).click();
    await expect(page.getByTestId('calendar-day-z-w3-2026-09-05').locator('[data-testid^="workout-card-"]')).toHaveCount(0);
  });
});
