...
async function expectNoAccentCellRing(day: Locator) {
  const style = await day.evaluate((el) => {
    const computed = getComputedStyle(el);
    return { boxShadow: computed.boxShadow, className: el.className };
  });
  expect(style.className.includes('ring-[var(--cal-accent)]')).toBeFalsy();
  expect(style.boxShadow.includes('rgb(0, 122, 255)')).toBeFalsy();
}
...
test('hover New session opens a dialog; cancel creates nothing', async ({ page, request }) => {
  ...
  await expect(page.getByTestId('new-session-dialog').locator('select')).toHaveCount(0);
  await expect(page.getByTestId('new-session-block')).toHaveAttribute('role', 'combobox');
  await expect(page.getByTestId('new-session-week')).toHaveAttribute('role', 'combobox');
  await page.getByTestId('new-session-title').fill('');
  const nameList = page.getByTestId('new-session-title-listbox');
  await expect(nameList).toBeVisible();
  await expect(nameList.getByRole('option', { name: 'Unlabeled' })).toBeVisible();
  await expect(nameList.getByRole('option', { name: 'Day 1' })).toHaveCount(0);
  await expect(nameList.getByRole('option', { name: 'day 1' })).toHaveCount(0);
  await expect(nameList.getByRole('option', { name: 'D1' })).toHaveCount(0);
  const covered = await nameList.evaluate((el) => {
    const rect = el.getBoundingClientRect();
    const x = rect.x + rect.width / 2;
    const y = rect.y + Math.min(rect.height - 2, 48);
    const hit = document.elementFromPoint(x, y);
    return Boolean(hit?.closest('[data-testid="new-session-title-listbox"]'))
      && !hit?.closest('[data-testid="new-session-day"]');
  });
  expect(covered).toBeTruthy();
  const listStyle = await nameList.evaluate((el) => {
    const computed = getComputedStyle(el);
    return { z: computed.zIndex, transform: computed.transform, bg: computed.backgroundColor };
  });
  expect(Number(listStyle.z)).toBeGreaterThanOrEqual(30);
  expect(listStyle.transform === 'none' || listStyle.transform === '').toBeTruthy();
  expect(listStyle.bg === 'transparent' || listStyle.bg === 'rgba(0, 0, 0, 0)').toBeFalsy();
  ...
  await expect(page.getByTestId('session-labels')).toContainText('Block Hypertrophy');
  await page.getByRole('button', { name: 'Back' }).click();
  await createDatedSession(page, '2026-09-06', 'Day 1');
  await page.getByRole('button', { name: 'Back' }).click();
  await page.getByTestId('calendar-day-2026-09-07').hover();
  await page.getByTestId('calendar-new-session-2026-09-07').click();
  await page.getByTestId('new-session-title').fill('');
  const laterNameList = page.getByTestId('new-session-title-listbox');
  await expect(laterNameList.getByRole('option', { name: 'Squat' })).toBeVisible();
  await expect(laterNameList.getByRole('option', { name: 'Unlabeled' })).toBeVisible();
  await expect(laterNameList.getByRole('option', { name: 'Day 1' })).toHaveCount(0);
  await page.getByTestId('new-session-cancel').click();
  await day.hover();
  ...
  const dest = page.getByTestId('calendar-day-2026-09-11');
  const destStyle = await dest.evaluate((el) => {
    const computed = getComputedStyle(el);
    return { className: el.className, boxShadow: computed.boxShadow };
  });
  expect(destStyle.className.includes('ring-[color-mix')).toBeTruthy();
  expect(destStyle.boxShadow === 'none' || destStyle.boxShadow === '').toBeFalsy();
  await dest.click();
  ...
  await expect(page.getByTestId('edit-session-dialog').locator('select')).toHaveCount(0);
...
test('hover overlay does not grow the day cell; Notes saves a day card', async ({ page, request }) => {
  ...
  if (await todayCell.count()) {
    await expectNoAccentCellRing(todayCell);
  }
  ...
  await expectNoAccentCellRing(day01);
  ...
  await expectNoAccentCellRing(day03);
