...
**Set `%`:** Column between `#` and Plan. Per-set signed integer percent (`dropPercent` in percentage points, e.g. `−10`). Architecture §6.7.2 **bar-load** drop from the lift’s **top/anchor kg** — not e1RM scaling, not a 7th Tab cell. Anchor: set 0 LOG kg if `actual > 0`, else set 0 `plannedWeight` if `> 0`. No anchor → do not invent kg; copy `Type Plan kg or log a set first.` On Enter / blur / stepper commit: that row’s `plannedWeight = roundToCompetitionPlates(anchor × (1 + pct/100))`. Skip write if result ≤ 0. Logged rows (LOG kg filled) still display stored `%` but do not rewrite Plan kg. Persist `dropPercent` on the set write path. Do not persist client-only `adjustment_pct`. Do not auto-write from e1RM. New rows start `dropPercent: 0` (`isAuto: false`); do not default extra sets to `−5`. Header: `%`. Cell: compact numeric, suffix `%`. Locked / readonly: display only. After a `%` commit, `use {n}` may still offer if the e1RM suggestion differs.
